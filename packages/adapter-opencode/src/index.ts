...
JSON array of stale indexes:`;
  ...
  const msgTokensMemo = makeMsgTokensMemo(msgTokenCache, TOKEN_CACHE_MAX);
...
<Your final concrete solution with code/commands/configs as needed.>`;
  ...
  const __transformDeps: any = {
    ...
    state: {
      get lastModelKey() { return lastModelKey; }, set lastModelKey(v) { lastModelKey = v; },
      get lastContextPercentage() { return lastContextPercentage; }, set lastContextPercentage(v) { lastContextPercentage = v; },
      get reasoningWatermark() { return reasoningWatermark; }, set reasoningWatermark(v) { reasoningWatermark = v; },
      get lastTailStartIdx() { return lastTailStartIdx; }, set lastTailStartIdx(v) { lastTailStartIdx = v; },
      get historianFailureCount() { return historianFailureCount; }, set historianFailureCount(v) { historianFailureCount = v; },
      get dissatisfactionCount() { return dissatisfactionCount; }, set dissatisfactionCount(v) { dissatisfactionCount = v; },
      get currentOpenCodeSessionId() { return currentOpenCodeSessionId; }, set currentOpenCodeSessionId(v) { currentOpenCodeSessionId = v; },
      get historianTurnCount() { return historianTurnCount; }, set historianTurnCount(v) { historianTurnCount = v; },
      get lastCompressTime() { return lastCompressTime; }, set lastCompressTime(v) { lastCompressTime = v; },
    },
  };

  // Extracted verbatim from the messages.transform hook (Phase-1 lift, behavior-zero-change).
  // Closure-scoped so it captures every dependency (getContextUsage, compartmentStore,
  // msgTokensMemo, reasoningWatermark, lastTailStartIdx, historianFailureCount, pinnedTags,
  // droppedTags, historian, pendingIdleWork, localLlmMode, ...) with ZERO injection — a pure
  // physical relocation. Do NOT change any logic here; this block has a history of
  // catastrophic bugs (#277 protectLine blowup, non-idempotency collapse, orphan tool_result 400s).
      ...
      neural_expand: tool({
        ...
        async execute(args) {
          if (args.start !== undefined && args.end !== undefined) {
            try {
              const expandSessionId = state.currentOpenCodeSessionId || sessionId;
...
List the angles in 1-2 sentences each. Be concise.`;
    ...
    "experimental.chat.messages.transform": magicContextPresent
      ? undefined
      : (input: any, output: any) => runCompartmentTransform(input, output, __transformDeps),
